...
Logs to both console and debug.log file. Respects DEBUG_MODE flag.
"""
...
class DebugLogger:
    ...
    def log_frame_start(self, frame_num, total_frames):
        """Log frame processing start (verbose, only in DEBUG_MODE)."""
    ...
    def log_frame_detections(self, frame_num, detections, inference_ms):
        """Log per-frame detection results. Respects DEBUG_MODE."""
        if config.DEBUG_MODE:
            num_vehicles = len(detections)
            self.logger.debug(f"  Frame {frame_num}: {num_vehicles} vehicles | Inference: {inference_ms:.1f}ms")
            for det in detections:
                self.logger.debug(
                    f"    ID={det['id']:3d} Class={det['class']:10s} BBox=({det['bbox'][0]:.0f},{det['bbox'][1]:.0f},"
                    f"{det['bbox'][2]:.0f},{det['bbox'][3]:.0f}) Centroid=({det['centroid'][0]:.0f},{det['centroid'][1]:.0f})"
                )

    def log_frame_summary(self, frame_num, total_vehicles, avg_inference_ms):
        """Log summary every LOG_INTERVAL frames (when DEBUG_MODE=False)."""
